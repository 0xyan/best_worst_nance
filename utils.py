...
def charts(a_periods, days_hours, kind, top_bottom, min_vol, percentage, top_df):
    ...
    plt.title(
        f"{kind} performers & BTC, last {a_periods} {days_hours}, Binance futures, volume > ${int(min_vol/1000000)}m, {top_bottom} {int(percentage*100)}%"
    )
    for asset in top_df:
        if asset != "BTCUSDT":
            plt.plot(
                top_df.index,
                top_df[asset],
                label=f"{asset} {round(top_df[asset][-1]*100, 2)}%",
            )
            plt.annotate(
                asset,
                xy=(0.95, top_df[asset][-1]),
                xytext=(8, 0),
