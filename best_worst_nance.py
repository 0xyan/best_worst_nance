...
async def futures_continous_klines(
    client, symbol, interval, start_str=None, limit=None
):
    df = pd.DataFrame()
    try:
        if start_str is not None and limit is not None:
            klines_data = await client.futures_continous_klines(
                pair=symbol,
                interval=interval,
            ...
            klines_data = await client.futures_continous_klines(
                pair=symbol, interval=interval, contractType="PERPETUAL", limit=limit
            )
        else:
...
async def ticker_volume_filtering(client, tickers, min_vol):
    ...
    async def process_ticker(ticker):
        try:
            df = await futures_continous_klines(
                client=client, symbol=ticker, interval="1d", limit=3
            )
...
async def df_creation(client, filtered_tickers, interval, start_str=None, limit=None):
    ...
    async def process_ticker(ticker):
        try:
            df_dict[ticker] = await futures_continous_klines(
                client, ticker, interval, start_str, limit
            )
...
def best_worst_coins(filtered_tickers, df_dict, percentage=0.2):
    ...
    if not filtered_tickers or not df_dict:
        raise ValueError("Empty input data")

    # Create dictionary of series first
    series_dict = {k: df_dict[k][k] for k in df_dict}
    # Use concat to create DataFrame at once
    cum_perf = pd.concat(series_dict, axis=1)

    # Limit to maximum 15 tokens per list
    max_tokens = min(15, int(round(len(filtered_tickers) * percentage, 0)))

    last_value_series = cum_perf[-1::].max()
    best_performers = round(last_value_series.nlargest(max_tokens) * 100, 2)
    worst_performers = round(last_value_series.nsmallest(max_tokens) * 100, 2)
...
def two_days():
    asyncio.run(
        main(
            min_vol=50000000, interval="15m", percentage=0.2, start_str=None, limit=192
        )
    )
...
def week():
    asyncio.run(
        main(min_vol=50000000, interval="1h", percentage=0.2, start_str=None, limit=168)
    )
